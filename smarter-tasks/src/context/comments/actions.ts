...
export let fetchComments = async (dispatch: CommentsDispatch, projectID : string, taskID : string) => {
  ...
  try {
    ...
    dispatch({ type: "FETCH_COMMENTS_FAILURE", payload: 'Unable to load members' });
  }
};
...
export const addComment = async (dispatch: CommentsDispatch, projectID : string, taskID : string, comment : Comment) => {
  try {
    const token = localStorage.getItem("authToken") ?? "";
    const response = await fetch(`${API_ENDPOINT}/projects/${projectID}/tasks/${taskID}/comments`, {
      method: 'POST',
